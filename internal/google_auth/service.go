...
func (g *GoogleProvider) Authenticate(code string) (types.UserInfo, *oauth2.Token, error) {
	...
	g.logger.Info().
		Str("access_token", token.AccessToken[:10]+"..."). // Частично скрываем для безопасности
		Str("refresh_token", token.RefreshToken).          // Полный refresh_token, если есть
...
func (g *GoogleProvider) ValidateRefreshToken(refreshToken, expectedEmail string) error {
	if refreshToken == "" {
